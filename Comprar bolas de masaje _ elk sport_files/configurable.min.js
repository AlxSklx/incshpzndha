...
var selectedId=$(this.options.selectedId).val();var parentSku=$(this.options.skus).data('parent');if(parentSku){var sku=parentSku.toString().replace('-parent','');}
if(selectedId&&sku){var modalForm=$('#query-product-modal');sku=$(this.options.skus).find('[data-id='+$(this.options.selectedId).val()+']').data('sku');$(this.options.reference).find('span').text(sku);if(modalForm.length){modalForm.find('#reference').val('Producto Ref:'+sku);}}
...
element.options[index].config=options[i];index++;}}}},_getOptionLabel:function(option){return option.label;},_clearSelect:function(element){var i;for(i=element.options.length-1;i>=0;i--){element.remove(i);}},_getAttributeOptions:function(attributeId){if(this.options.spConfig.attributes[attributeId]){return this.options.spConfig.attributes[attributeId].options;}},_reloadPrice:function(){$(this.options.priceHolderSelector).trigger('updatePrice',this._getPrices());},_getPrices:function(){var prices={},elements=_.toArray(this.options.settings),hasProductPrice=false;_.each(elements,function(element){var selected=element.options[element.selectedIndex],config=selected&&selected.config,priceValue={};if(config&&config.allowedProducts.length===1&&!hasProductPrice){priceValue=this._calculatePrice(config);hasProductPrice=true;}else{this._renderPercentageDiscount(false);}
prices[element.attributeId]=priceValue;},this);return prices;},_calculatePrice:function(config){var displayPrices=$(this.options.priceHolderSelector).priceBox('option').prices,newPrices=this.options.spConfig.optionPrices[_.first(config.allowedProducts)],percentajeDiscount=false;if("oldPrice"in newPrices){percentajeDiscount=this._calculateDiscountPercentage(newPrices);}
if(percentajeDiscount){this._renderPercentageDiscount(percentajeDiscount);}
_.each(displayPrices,function(price,code){if(newPrices[code]){displayPrices[code].amount=newPrices[code].amount-displayPrices[code].amount;}});return displayPrices;},_calculateDiscountPercentage:function(prices){if(prices['basePrice'].amount!==prices['oldPrice'].amount){var discount=prices['oldPrice'].amount-prices['basePrice'].amount;return(Math.round((discount*100)/ prices['oldPrice'].amount));}
return false;},_renderPercentageDiscount:function(amount){var offerTag=$('#product-main').find('.offer');if(offerTag.length){if(amount===false){amount=offerTag.find('.amount').data('minimal');}
offerTag.find('.amount').text(amount);}},_getSimpleProductId:function(element){var allOptions=element.config.options,value=element.value,config;config=_.filter(allOptions,function(option){return option.id===value;});config=_.first(config);return _.isEmpty(config)?undefined:_.first(config.allowedProducts);},_displayRegularPriceBlock:function(optionId){if(typeof optionId!='undefined'&&this.options.spConfig.optionPrices[optionId].oldPrice.amount!=this.options.spConfig.optionPrices[optionId].finalPrice.amount){$(this.options.slyOldPriceSelector).show();}else{$(this.options.slyOldPriceSelector).hide();}},_onGalleryLoaded:function(element){var galleryObject=element.data('gallery');this.options.mediaGalleryInitial=galleryObject.returnCurrentImages();}});return $.mage.configurable;});